...
/// 明文层压缩工具（dev target）。与 iOS 侧同 codec（LZFSE）、同默认参数。
enum RelayDialoutCompression {
    ...
    ///
    /// 实现说明（与 iOS 侧一致，Task 3 已验证）：本 SDK 的流式 `compression_stream_process` 无法解码
    /// `compression_encode_buffer` 生成的 LZFSE 块（`process` 首调用即返回 `COMPRESSION_STATUS_ERROR`），
    /// 因此改用与 `compress` 对称的一次性 `compression_decode_buffer`，语义不变（同 LZFSE、输出硬上限、bomb 保护）。
