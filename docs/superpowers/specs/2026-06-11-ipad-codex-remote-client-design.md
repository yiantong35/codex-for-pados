...
### D7：UI 复刻三栏，右栏 inspector 可隐藏（复刻 desktop 开关）
两列 `NavigationSplitView`（左：项目/对话｜中：对话流）+ iOS 17 `.inspector` 右栏（v1 简态：cwd/分支/模型）。复刻 desktop 右上角面板开关：**inspector 默认隐藏，由 content 工具栏的切换按钮显隐**。**全局设置齿轮放侧栏 `.toolbar`**（侧栏在 iPad 常驻，保证连接后始终可见，不随 inspector 显隐丢失）。进入主界面默认聚焦侧栏，未选中对话时中栏不显大占位卡（最小空态）。富态环境信息（git/进度/任务/浏览器）留 v2+。竖屏侧栏抽屉化。
