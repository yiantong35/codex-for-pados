...
**目标：** 构建一个 iPadOS 原生 SwiftUI app，经 SSH 隧道连接 Mac 上仅绑 loopback 的 `codex app-server`（JSON-RPC 2.0 over WebSocket），复刻 Codex 桌面 app 的左栏（项目→对话树）+ 中栏（流式正文/命令输出/文件 diff/多选项审批卡），并支持恢复桌面会话、图片附件、模型/推理可调、转向(steer)/排队/中断。

**架构：** 五层自下而上 — 传输层（Citadel SSH direct-tcpip 端口转发 + `URLSessionWebSocketTask`，actor）→ JSON-RPC 层（id 关联 + 通知 async 流 + server→client 请求分发，actor）→ 领域/归约层（schema 派生的 `Codable` 类型 + `ThreadReducer`）→ 状态层（`@Observable` Stores）→ SwiftUI 三栏 `NavigationSplitView`。协议类型由 `codex app-server generate-json-schema/generate-ts` 生成（pin codex 0.133.0），协议层隔离以便升级。

**技术栈：** Swift 6 / SwiftUI / Observation 框架 / `URLSessionWebSocketTask` / [Citadel](https://github.com/orlandos-nl/Citadel)（SPM，基于 Apple swift-nio-ssh，支持 TCP 转发）/ XCTest / 录制 fixture + mock 传输做单元测试。

**设计文档：** 详见 `docs/superpowers/specs/2026-06-11-ipad-codex-remote-client-design.md`。本计划是该设计的可执行展开，任务按依赖排序，**技术验证 spike 在最前**（设计 §11 风险最大项：Citadel 在 iPadOS 端口转发稳定性未知）。
...
Task 3  ★SPIKE★ Citadel SSH 端口转发 + initialize 握手（最大未知风险，先打通）
Task 4  协议层：JSON-RPC 信封 Codable + 编解码（fixture 单测）
Task 5  协议层：MVP 领域类型 Codable（initialize/thread/turn/approval）
Task 6  传输层：SSHTunnel（封装 spike 成果为可复用 actor）
Task 7  传输层：WebSocketTransport（actor，原始 JSON 帧收发）
...
ios/CodexRemote/
  ...
  Transport/      # SSHTunnel、WebSocketTransport
...
**对应 spec：** `mac-launcher/spec.md` 全部场景（正常启动并输出连接信息 / sshd 未开启提示 / 端口被占用 / codex 版本不符合 pin）。
...
# 3) 端口占用检测（mac-launcher: 端口被占用）
if lsof -nP -iTCP:"$PORT" -sTCP:LISTEN >/dev/null 2>&1; then
  echo "❌ 端口 $PORT 已被占用，可能已有 app-server 实例。中止，不启动重复实例。" >&2
  exit 1
fi
```
...
```bash
# 4) 输出连接信息（mac-launcher: 正常启动并输出连接信息）
LAN_IP="$(ipconfig getifaddr en0 2>/dev/null || ipconfig getifaddr en1 2>/dev/null || echo '未知')"
SSH_USER="$(whoami)"
echo "──────────────────────────────────────────"
echo " Codex app-server 即将启动（仅绑 127.0.0.1）"
echo "  Mac LAN IP : $LAN_IP"
echo "  监听端口   : $PORT (loopback only)"
echo "  SSH 用户名 : $SSH_USER"
echo "  iPad 连接  : 主机=$LAN_IP  端口=$PORT  SSH用户=$SSH_USER"
echo "──────────────────────────────────────────"
```
...
```bash
# 5) 启动（仅绑 loopback），优雅退出清理
cleanup() { echo; echo "已停止 app-server。"; }
trap cleanup EXIT INT TERM
exec codex app-server --listen "ws://127.0.0.1:${PORT}"
```
...
# 场景A 正常：
./scripts/start-codex-appserver.sh 8473
```

Expected：打印 LAN IP / 端口 8473 / SSH 用户名，随后 app-server 进入监听（Ctrl-C 退出打印“已停止”）。

```bash
# 场景C 端口占用：保持上一个运行，另开终端再跑同端口
./scripts/start-codex-appserver.sh 8473
```

Expected：打印“端口 8473 已被占用…中止”，退出码非 0。
...
git commit -m "feat(mac-launcher): one-shot script to start loopback-only codex app-server with preflight checks"
```

---

## Task 3：★SPIKE★ Citadel SSH 端口转发 + initialize 握手

**对应 spec：** `remote-connection/spec.md`「建立隧道并完成握手」（spike 阶段只验证连通性，不做完整 UI）。

**这是 build 首步必须打通的最大未知风险**（设计 §11：Citadel 在 iPadOS 端口转发的可靠性）。先用最小可运行 demo 验证 `direct-tcpip` 转发 + 经隧道完成 `initialize`→`initialized` 握手，再铺开 Task 6/7/8。
...
struct SpikeView: View {
    ...
    @State private var appServerPort = "8473"
    ...
    var body: some View {
        Form {
            ...
            TextField("app-server 端口", text: $appServerPort)
                ...
                Task {
                    ...
                    do {
                        log = try await runner.run(
                            ...
                            appServerPort: Int(appServerPort) ?? 8473,
...
- [ ] **Step 2：写 spike 运行逻辑（SSH + direct-tcpip + WS + initialize）**

`ios/CodexRemote/Spike/SpikeRunner.swift` —— 用 Citadel 建 SSH，开 `direct-tcpip` 转发到 `127.0.0.1:<appServerPort>`，在转发通道上跑一个最小 WebSocket 升级并发送一条 `initialize` JSON-RPC，收到响应后发 `initialized` 通知：
...
struct SpikeRunner {
    func run(host: String, sshPort: Int, appServerPort: Int,
        ...
        // 2) direct-tcpip 转发：本地发起到 Mac 127.0.0.1:appServerPort 的逻辑通道
        //    Citadel 暴露 createDirectTCPIPChannel 之类 API；以实际 API 为准。
        //    spike 目标：拿到一个可读写的 channel，承载 WS 帧。
        let channel = try await client.createDirectTCPIPChannel(
            using: SSHChannelType.DirectTCPIP(
                targetHost: "127.0.0.1", targetPort: appServerPort,
                originatorAddress: try .init(ipAddress: "127.0.0.1", port: 0)))

        // 3) 在该 channel 上做最小 WebSocket 升级 + 发送 initialize。
        //    spike 阶段可直接在 channel 上手写最小 HTTP Upgrade + 一帧文本，
        //    或验证“能否在转发通道上承载 URLSessionWebSocketTask”。
        //    关键验证点：握手 JSON 能发出且能收到 initialize 响应。
        let initialize = """
        {"jsonrpc":"2.0","id":1,"method":"initialize","params":{"clientInfo":{"name":"CodexRemote-Spike","title":null,"version":"0.0.1"},"capabilities":null}}
        """
        // 经 channel/WS 写出 initialize，等首个响应（result 含 userAgent/codexHome）。
        let response = try await SpikeWire.sendAndAwaitFirstResponse(
            channel: channel, payload: initialize)
        // 发 initialized 通知（无 id）
        try await SpikeWire.send(channel: channel,
...
> `SpikeWire` 是 spike 内联的极小帮手（一个 enum + 两个静态 async 方法），负责把字符串经 channel/WS 写出并读回第一条完整 JSON。spike 不追求工程化，只要**真机上能跑通握手**。若 Citadel 的 direct-tcpip API 命名/签名与上文不符，以 Citadel 当前版本源码/文档为准，但语义不变：建立到 `127.0.0.1:<appServerPort>` 的转发通道。
...
前置：在 Mac 上跑 `./scripts/start-codex-appserver.sh 8473`（Task 2）。
Run：Xcode 选**真实 iPad**（非模拟器，转发行为可能不同）→ 运行 → 在 SpikeView 填主机/端口/用户/密码 → 点「连接并握手」。
Expected：日志区显示「握手成功 ✅」并打印含 `userAgent` / `codexHome` 的 initialize 响应。

> 若失败：加载 **superpowers:systematic-debugging** skill 定位（Citadel 转发是否建立？WS 升级是否成功？JSON 是否被 app-server 接受？）。**根因未定位前不得继续 Task 6+**，因为后续传输层完全建立在此之上。

- [ ] **Step 5：记录 spike 结论**

在 `ios/CodexRemote/Spike/SpikeRunner.swift` 顶部用一行注释记录验证结论（如：`// SPIKE PASS 2026-06-11: Citadel direct-tcpip + WS initialize 握手在 iPad 真机跑通`），并记录任何 API 偏差，供 Task 6 复用。

- [ ] **Step 6：Commit**

```bash
git add ios/CodexRemote/Spike ios/CodexRemote/App/CodexRemoteApp.swift
git commit -m "spike: verify Citadel direct-tcpip forwarding + app-server initialize handshake on iPad"
```
...
- [ ] **Step 2：写 initialize 类型（取自 InitializeParams.ts / InitializeResponse.ts / ClientInfo.ts）**
...
- [ ] **Step 3：写 thread 类型（取自 v2/Thread.ts, ThreadListParams.ts, ThreadListResponse.ts, ThreadResumeParams.ts, ThreadStartParams.ts, ThreadSourceKind.ts）**
...
- [ ] **Step 4：写 turn 类型（取自 v2/TurnStartParams.ts, TurnSteerParams.ts, UserInput.ts, NonSteerableTurnKind.ts, ImageDetail.ts）**
...
- [ ] **Step 5：写审批类型（取自 schema：CommandExecutionRequestApprovalResponse.json, FileChangeRequestApprovalResponse.json, ExecCommandApprovalResponse.ts → ReviewDecision.ts, CommandExecutionRequestApprovalParams.json, FileChangeRequestApprovalParams.json）**
...
- [ ] **Step 6：写解码/编码往返测试（用真实 schema 形状）**
...
**对应 spec：** `remote-connection/spec.md`「建立隧道并完成握手」「SSH 鉴权失败」「app-server 不可达」。依赖 Task 3 spike 结论与 Task 5 类型。

**Files:**
- Create: `ios/CodexRemote/Transport/SSHTunnel.swift`
...
enum TransportError: Error, Equatable {
    ...
    case wsClosed(code: Int, reason: String?)
    case notConnected
}
```

- [ ] **Step 2：实现 SSHTunnel actor（建连 + direct-tcpip 转发，复用 spike 中已验证的 Citadel API）**

`ios/CodexRemote/Transport/SSHTunnel.swift`：
...
actor SSHTunnel {
    ...
    /// 建立 SSH 连接并返回到 Mac 127.0.0.1:appServerPort 的 direct-tcpip channel。
    /// 鉴权失败 → TransportError.sshAuthFailed；连接成功但转发不可用 → appServerUnreachable。
    func connect(host: String, sshPort: Int, appServerPort: Int,
                 auth: SSHAuth) async throws -> Channel {
...
> 以 Task 3 spike 中确认可用的 Citadel API 形状为准。若 spike 验证 WS 须经转发 channel 手写帧（而非 channel 直连），此处仅负责拿到 channel，WS 升级在 Task 7。
...
```bash
git add ios/CodexRemote/Transport/SSHTunnel.swift ios/CodexRemote/Transport/TransportError.swift
git commit -m "feat(transport): SSHTunnel actor with direct-tcpip forwarding + typed errors"
```
...
**对应 spec：** `remote-connection/spec.md`（WebSocket 承载 JSON-RPC）。依赖 Task 6 的转发 channel。

定义一个传输抽象协议，便于 Task 8/10 用 mock 替身做单元测试（设计 §10）。

**Files:**
- Create: `ios/CodexRemote/Transport/MessageTransport.swift`（抽象协议）
- Create: `ios/CodexRemote/Transport/WebSocketTransport.swift`
...
/// 收发原始 JSON 文本帧的抽象。真实实现走 WS over SSH，测试用 mock。
...
- [ ] **Step 2：实现 WebSocketTransport actor**

`ios/CodexRemote/Transport/WebSocketTransport.swift` —— 在 Task 6 的转发 channel 上承载 WebSocket。spike 已确认承载方式（`URLSessionWebSocketTask` 或经 channel 手写 WS 帧），此处按 spike 结论实现 `send`/`incoming`：
...
actor WebSocketTransport: MessageTransport {
    ...
    func send(_ text: String) async throws {
        // 按 spike 结论：把 text 作为一帧 WS 文本写入 channel。
    ...
    func incoming() -> AsyncThrowingStream<String, Error> {
        AsyncThrowingStream { continuation in
            ...
            // 安装 inbound handler，把收到的每一条完整 JSON 文本帧 yield 出去。
            // 具体 handler 接线按 spike 中验证可行的方式实现。
        }
    }
...
> WS 帧的具体读写（升级、分帧、文本帧解析）以 spike 中真机验证可行的实现为准。关键契约：`send` 发一条完整 JSON 文本，`incoming()` 每条完整 JSON 文本帧 yield 一次。
...
```bash
git add ios/CodexRemote/Transport/MessageTransport.swift ios/CodexRemote/Transport/WebSocketTransport.swift ios/CodexRemoteTests/MockTransport.swift
git commit -m "feat(transport): MessageTransport protocol + WS-over-SSH transport + MockTransport"
```
    ...
    private func handle(_ line: String) async {
        ...
        case .error(let e):
            pending.removeValue(forKey: e.id)?
                .resume(throwing: TransportError.forwardingFailed(e.error.message))
...
**对应 spec：** `remote-connection/spec.md`「建立隧道并完成握手」「SSH 鉴权失败」「app-server 不可达」「连接中断后自动重连」「重连期间的用户可见状态」。依赖 Task 6/7/8。
...
struct ConnectionConfig: Sendable {
    var host: String; var sshPort: Int; var appServerPort: Int; var auth: SSHAuth
    static var stub: ConnectionConfig {
        .init(host: "x", sshPort: 22, appServerPort: 8473, auth: .password(user: "u", password: "p"))
    }
}

enum ConnectionPhase: Equatable {
    case disconnected, sshConnecting, forwarding, wsConnecting, initializing, ready
...
final class ConnectionStore {
    ...
    private func establish(_ config: ConnectionConfig) async throws {
        phase = .wsConnecting
        let transport = try await transportFactory(config)     // 工厂内部含 SSH+转发+WS
...
- [ ] **Step 4：把真实工厂接线（SSH→转发→WS）放在 App 装配处**
...
func liveTransportFactory(_ config: ConnectionConfig) async throws -> MessageTransport {
    ...
    let channel = try await tunnel.connect(
        host: config.host, sshPort: config.sshPort,
        appServerPort: config.appServerPort, auth: config.auth)
    return WebSocketTransport(channel: channel)
}
```
...
**对应 spec：** `remote-connection/spec.md`「凭证存入 Keychain」+「建立隧道并完成握手」（用户输入连接信息）。依赖 Task 10。
...
struct ConnectionConfigView: View {
    ...
    private func connect() async {
        ...
        let cfg = ConnectionConfig(host: host, sshPort: Int(sshPort) ?? 22,
                                   appServerPort: Int(appServerPort) ?? 8473, auth: auth)
        do { try await connection.connect(config: cfg); errorText = nil }
        catch TransportError.sshAuthFailed(let m) { errorText = "SSH 鉴权失败：\(m)" }
        catch TransportError.appServerUnreachable { errorText = "app-server 不可达，请检查 Mac 端启动脚本是否运行。" }
...
在 Mac 跑 `./scripts/start-codex-appserver.sh 8473`（Task 2）；iPad 与 Mac 同 LAN；Xcode 选真实 iPad 运行 app；用 `ConnectionConfigView` 填真实 SSH 凭证连接。
...
操作 A：连接后断开 iPad WiFi 几秒再恢复 → 观察「重连中」横幅 → 自动重建隧道 + `initialize` + 对当前线程 `thread/resume`。
操作 B：故意填错 SSH 密码连接 → 应显示明确「SSH 鉴权失败」而非泛化错误。
操作 C：SSH 通但不跑 app-server → 应显示「app-server 不可达」。
...
**2. Placeholder 扫描：** 各代码步骤均含完整可运行代码；字段名取自真实 schema；测试均含具体断言。唯一标注「以 spike/真实帧为准」处（WS 帧读写细节、若干 notification 字段名）属设计 §13 明确留待 build 确认项，已在 Task 3/20 设校正点，非 placeholder。
...
**核心约束落实确认：** build 首步 = Task 3 Citadel spike（最大风险前置）✅；协议从 generate 生成入仓库 + pin 0.133.0（Task 1）✅；五层分层（Task 4-13）✅；审批多选项 + v2 主 + legacy 兼容 + serverRequest/resolved + 超时不自动批准（Task 18/19）✅；协议层/归约层 fixture+mock 单测、SSH/WS 靠 spike+E2E（贯穿）✅。
