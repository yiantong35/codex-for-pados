...
- [x] **Step 1: 临时把 detail 改成「中栏 .inspector 占位 + 外层全宽下栏」最小形态**
...
- [x] **Step 5: 回退 spike 临时改动（spike 不进主线）**
...
- [x] **Step 1: 把 `detail` 改为中栏内容直接挂 inspector（不再走 WorkspaceDetailRegion 的右栏 HStack）**
...
- [x] **Step 3: 把 inspector 的 `isPresented` 真正绑到 `RootSplitView.$showRightPanel`（让顶栏右按钮 toggle 生效）**
...
- [x] **Step 1: detail 去掉 VStack/下栏，只留中栏 inspector**
...
- [x] **Step 2: body 在 split 外层挂全宽下栏 safeAreaInset（与顶栏对称）**
...
- [x] **Step 2: 从 `WorkspaceMetrics.swift` 删除 `resizedRightWidth`**
...
- [x] **Step 3: 从 `WorkspaceMetricsTests.swift` 删除引用 `resizedRightWidth` 的 4 个测试**
...
- [x] **Step 3: 编译验证（xcodegen 按目录收文件，删后自动不再编译）**
...
- [x] **Step 2: 逐态截图自检（静态可验，tasks.md 5.2 + 4.x）**
...
- [x] 顶栏工具栏 5 按钮（左/下/摘要/右/设置）在位；右按钮切 inspector 显隐生效（tasks.md 4.1）。
- [x] 摘要 `:≡` 浮层正常浮现、内容（P0 数据）在（tasks.md 4.2）。
- [x] 橙 `AccentColor` 主题：下栏把手/左把手 hover 或拖动态为橙（tasks.md 4.3）。
- [x] 左栏选中态自渲染、左把手宽度监听高亮在（tasks.md 4.3）。
- [x] `SettingsMenu` 与 composer 模型选择 `.popover` 能弹（tasks.md 4.4）。
- [x] 右栏 / 下栏空态占位（`PanelEmptyState`）在（tasks.md 4.5）。
- [x] 收起左 sidebar 时右栏 inspector 仍在、可独立显隐（spec.md「不受左边栏显隐影响」）。

- [x] **Step 3: 暂停 → 用户实测验收（CLI 验不了，tasks.md 5.3）**
...
- [x] **Step 4: 真机验收延期登记（tasks.md 5.4）**
