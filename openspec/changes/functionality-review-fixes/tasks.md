...
- [ ] 1.1 新增 RelayProtocol 共享 wire limit；relay-server、dialout upgrader、iOS ImageEncoder/RelayTransport 全部引用；真实 NIO 端到端测试覆盖 16 KiB < frame <= 1 MiB，最终序列化帧超限在发送前显式失败。
