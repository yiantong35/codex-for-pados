...
- [x] 2.6 D7 图标按钮（ComposerView 图片/模型/停止/发送等）统一 ≥44pt 命中框 + `.accessibilityLabel`；`SidebarView` 会话行 `onTapGesture` 改 `Button`（button trait + 键盘激活），保留视觉
- [x] 2.7 D7 测试/验收：VoiceOver 语义标签存在；命中框 ≥44pt；会话行键盘可激活
...
- [x] 3.4 D9 收口检查：所有新增/改动路径有断言级测试；无仅「PNG 非空」的空快照断言残留
