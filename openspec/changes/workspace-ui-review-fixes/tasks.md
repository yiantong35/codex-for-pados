...
- [ ] 1.3 D2 `ConnectionStore` resumeHandler 单一属性 → token 订阅表；`addResumeHandler`(返回 token，保留首连恰一次补触发，订阅者维度化 `didInitialRejoin`) / `removeResumeHandler(token)`；物理重连 `.ready` 遍历触发全部
- [ ] 1.4 D2 迁移调用点：`ConversationView` `.task` 用 add→（onDisappear/defer）remove 配对；`setResumeHandler` 保留为薄封装或全量迁移
- [ ] 1.5 D2 单测：多订阅者互不覆盖；首连对已 ready 的新订阅者补触发恰一次；重连遍历触发全部；注销后不再触发
