...
折叠的项目分组 SHALL 在分组行显示该组内「需注意」会话数徽标，复刻 desktop 语义（运行中 / 待批准 / 待响应），而非会话总数；计数为 0 时不显示。

> 说明：unread 追踪需本地已读状态，留 v2；v1 计数覆盖待批准 + 运行中。

#### Scenario: 折叠分组显示需注意计数
- **WHEN** 某项目分组处于折叠态且组内存在待批准或运行中的会话
- **THEN** 该分组行显示等于「需注意」会话数的数字徽标
- **AND** 组内无需注意会话时不显示该徽标
