...
iPad 客户端 SHALL 提供右边栏整列面板，支持显隐切换、可拖拽改宽、最小宽度；本期内容为占位空态（真实内容由后续 change 提供）。拖拽改宽 SHALL 不受左边栏显隐影响，并在可拖边缘提供常驻的拖动提示。

#### Scenario: 右边栏显隐与拖动
- **WHEN** 用户切换右面板按钮
- **THEN** 右边栏整列在显示与隐藏间切换
- **AND** 显示时可拖拽改宽，且不小于最小宽度
- **AND** 拖拽改宽不受左边栏显隐影响（左边栏开着时右边栏仍可拖动）

#### Scenario: 右边栏拖动提示
- **WHEN** 右边栏显示
- **THEN** 其左缘常驻可见的拖动把手，提示该边缘可拖拽改宽
- **AND** 指针（触控板）悬停于把手时，把手加粗并高亮为主题色
