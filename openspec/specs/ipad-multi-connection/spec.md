...
### Requirement: tab 通知指示圆点

系统 SHALL 在每个 tab 上以单个圆点聚合状态。当连接就绪(`.ready`)时,圆点聚合该 Session 内所有会话的最高优先级状态,优先级从高到低为:系统错误(红,闪烁)> 等待审批或等待输入(橙,闪烁)> 运行中 active(绿,常亮)> 有未读活动(蓝,常亮)> 无。当连接**处于异常态**(phase 非 `.ready`,含重连中/失败/断线)时,该 tab SHALL 显示**灰点**表达连接异常,使断线在 tab 层可见;此灰点与红点(会话 `systemError`)语义正交——红点仅在 `.ready` 前提下由会话系统错误触发,灰点仅由连接异常触发,二者互不污染。仅"尚未发起连接"(未建 Session 的懒连态)的 tab SHALL NOT 显示圆点。tab 圆点 SHALL 与会话行徽标共享颜色语义但不共享形状:会话行保留既有图标,tab 用圆点。后台 tab 的圆点 SHALL 由保留的列表级广播实时更新。

#### Scenario: 聚合取最高优先级

- **WHEN** 某 tab 连接就绪且其 Session 同时有会话在运行且有会话等待审批
- **THEN** 该 tab 圆点显示等待审批(橙,闪烁),不被运行态稀释

#### Scenario: 断线 tab 显示灰点

- **WHEN** 某 tab 连接 phase 非 `.ready`(重连中/失败/断线)
- **THEN** 该 tab 显示灰点表达连接异常(而非透明无点)

#### Scenario: 未连接 tab 无圆点

- **WHEN** 某 tab 尚未发起连接(未建 Session 的懒连态)
- **THEN** 该 tab 不显示圆点

#### Scenario: 后台 tab 圆点实时更新

- **WHEN** 某后台机器的会话进入等待审批且其连接就绪
- **THEN** 对应 tab 的圆点实时变为等待审批(橙,闪烁)
