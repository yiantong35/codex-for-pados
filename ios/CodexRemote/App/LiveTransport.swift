...
func validatedRelayTOFUKey(_ value: String?) throws -> (key: String, machineId: UUID) {
    guard let value, let machineId = UUID(uuidString: value) else {
        throw TransportError.proxyFailed("Invalid relay trust identity")
    }
...
func liveTransportFactory(_ config: ConnectionConfig) async throws -> MessageTransport {
    ...
    let tofuIdentity = try validatedRelayTOFUKey(config.relayTOFUKey)
    // 建连前 peek（不删）：失败可用同一 pc 直接重试不重扫（D3）。受信任复连无 pc → nil → 空 proof。
    let pc = PendingPairingStore.shared.peek(for: tofuIdentity.machineId) ?? ""
    ...
    let consume: @Sendable () async -> Void = {
        await MainActor.run { _ = PendingPairingStore.shared.take(for: tofuIdentity.machineId) }
    }
    return try await makeRelayTransport(payload: payload,
                                        tofuMachineKey: tofuIdentity.key,
